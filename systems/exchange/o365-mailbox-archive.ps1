# Description:  Powershell script to archive TERMINATED employees O365 mailboxes exporting them to a PST file located on the C: drive
# Written By:  Chris Meader
# Initial Developed Date:  05/14/2024
# Date Last Modified:  05/15/2024
...
$Root_Disk = "<enter_root_disk>"
$PSTArchive = "$($Root_Disk)\<enter_folder_name>"
$SharePath = "<enter_share_drive_path>"
...
if (!(Test-Path "<enter_log_path>")) { New-Item "<enter_log_path>" -ItemType Directory }

#specify Log path
$log_directory = "<enter_log_path>\$(($script_name).Substring(0,(($script_name).length-4)))"
...
Connect-ExchangeOnline -UserPrincipalName <enter_email@domain.com>

Write-Host "Connect to Exhcnage Security `n`n"
Connect-IPPSSession -UserPrincipalName <enter_email@domain.com>
...
foreach ($user in $TerminatedUsers) 
{
	...
	try {
		...
		try {
            Write- "Exporting Mailbox to pst for $($user.UserPrincipalName) `n`n"
			...
			Get-ADUser $user.SamAccountName | Set-ADObject -Server $pdc-Server $pdc -Replace @{extensionAttribute4 = "Exported"}
		}
		catch {
